...
当前轮次状态或推进接口返回后，浏览器立即后台预加载图片，与状态及历史读取并行。加载队列最多并发 3 张，重复轮询和重播复用同一资源；缓存最多保留 24 张，播放前再次检查解码。所有必需大图与背景全部就绪后才启动时间轴，不边播放边补图。等待超过 12 秒或任一图片损坏则进入可读摘要，允许重试演出或直接继续推理；等待本身不会启动 AI 发言。后台暂停、减少动态效果和无效配置都有降级。演出可暂停或跳至结尾，结束后等待玩家点击“继续推理”。没有演出的剧本沿用原流程。

最后一个不含线索图片的镜头作为收尾字幕：图片可以提前收拢，上一句仍在原位淡出，收尾句出现时才居中。

v2 将每个镜头里的线索按顺序分配出现时间，组合宽景、偏轴近景与特写裁切。照片内部持续移动，遮幅逐渐打开，焦点由模糊转清晰；前后照片短暂交叠，再退至暗处。cinematic 采用横向显影与冷色光影，dossier 采用纵向揭页及轻微平面透视。结尾所有照片汇聚至中心，保留确认后开始推理的流程。图片、背景、文字、遮幅和焦点统一由可暂停时钟驱动；没有播放进度条、装饰轨迹、图片编号或脚注。演出期间禁止文字选择和元素拖动，结束后的完整线索仍可选择复制。手机有独立画幅与焦点。v2 的 `shots` 决定顺序、时长和短标题，`motion/labels/emphasis` 仅供 v1 使用。建议 v2 使用简短氛围文字，将详细证据留在结束页，不要求观众在播放期间阅读正文。
