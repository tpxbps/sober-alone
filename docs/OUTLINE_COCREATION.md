...
- 最后只整理一次，正文从流式草稿原位转为持久消息，再显示完成卡。用户可全文编辑或确认进入初稿；大纲阶段不调用审阅模型，不自动修订，不提供重新整理按钮。
...
其他 action：`stop_questions`、`rewrite`、`retry`；`pause`、`continue` 保留给旧客户端和历史任务恢复，正常界面不发送暂停请求。
