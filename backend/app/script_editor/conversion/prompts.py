...
## 固定真相
忠实使用终稿中确定的案件真相，不在揭晓阶段补充推理必需的新事实。结局模式为single时，ending_branches为空；multiple时，忠实提取终稿的四种后续结局（correct正确指认、incorrect错误指认、tie平票、no_votes无有效票），并填ending_culprit_name。案件事实与真凶不能随结果改变。禁止平票重投、额外讨论、隐藏行动或临场LLM判定。开场、简介和公开角色职业不能泄露私人身份。
