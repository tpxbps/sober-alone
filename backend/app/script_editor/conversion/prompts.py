...
调用提供的结构化函数返回结果，列表必须使用 JSON 数组，不能把列表写成字符串。
例如一轮的 free_speech_limits 为 [2]，两轮为 [2, 2]；只生成实际配置的轮次。"""
...
只依据上面的个人稿提炼约200-350字，短本可更短。按身份、亲历、重要关系自然分段。
不强制加入疑问或规定玩家要问谁；只有本人经历确实留下疑问时才自然表达。
