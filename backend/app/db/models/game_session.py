...
class GameSession(Base):
    ...
    status: Mapped[str] = mapped_column(
        String(20), default=lambda: GameStatus.WAITING.value
    )
    current_stage: Mapped[str] = mapped_column(
        String(30), default=lambda: GameStage.INTRO.value
    )
    ...
    # 当前发言角色
    current_speaker: Mapped[Optional[str]] = mapped_column(String(36), nullable=True)
    ...
    # 最终投票结果
    final_suspect_id: Mapped[Optional[str]] = mapped_column(String(36), nullable=True)
    final_suspect_name: Mapped[Optional[str]] = mapped_column(String(50), nullable=True)
    vote_result: Mapped[dict] = mapped_column(JSON, default=dict)  # 投票统计结果

    # MVP评选
    mvp_character_id: Mapped[Optional[str]] = mapped_column(String(36), nullable=True)
    mvp_reason: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
