...
test('斜杠选择公开线索并在发言气泡展示可访问引用', async ({ page }) => {
  ...
  await expect(optimisticRecord.getByLabel('查看线索 窗台红泥', { exact: true })).toBeVisible()
  ...
  const citation = persistedRecord.getByLabel('查看线索 窗台红泥', { exact: true })
  ...
  // Clicking has no toggle/pin behavior: leaving either surface always dismisses.
  await citation.click()
  await expect(clueTooltip).toBeVisible()
  await page.mouse.move(0, 0)
  await expect(clueTooltip).toBeHidden()
  await citation.dispatchEvent('click')
  await expect(clueTooltip).toBeHidden()
})
