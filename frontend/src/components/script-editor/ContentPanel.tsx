    ...
    if (isComplete && !assetHasFailures) {
      return (
        <div className="h-full flex flex-col items-center justify-center gap-4 p-6">
          ...
          {ownerUuid && (
            <div className="p-3 bg-secondary/50 rounded-lg w-full max-w-sm">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={() => {
                    navigator.clipboard.writeText(ownerUuid);
                    setCopied(true);
...
// =============================================================================
// Convert Progress (multi-step LLM structured output)
// =============================================================================

function ConvertProgressPanel({
  convertProgress,
  onRetry,
}: {
  convertProgress: AssetProgress | null;
...
// =============================================================================
// Asset Generation Progress (Issue 3 — task tree with phases)
// =============================================================================

function AssetGenerationProgress({
  assetProgress,
  onRetry,
}: {
  assetProgress: AssetProgress | null;
...
function TaskRow({
  ...
  task: { id: string; label: string; status: string };
  onRetry?: (taskId: string) => Promise<void>;
}) {
  return (
    <div
