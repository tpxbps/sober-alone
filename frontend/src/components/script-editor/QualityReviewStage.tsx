  ...
  return <div className="h-full flex flex-col">
    ...
    <div className="flex-1 min-h-0 overflow-y-auto p-4 space-y-3">
      ...
      {report.findings.map((finding, index) => <article key={index} className="rounded-lg border border-border p-4 space-y-2 text-sm">
        <div className="flex flex-wrap gap-2"><span className="text-warning">{{ critical: "严重", major: "重要", minor: "建议" }[finding.severity]}</span><span className="text-xs text-muted-foreground break-all">{finding.field}</span></div>
    ...
    <div className="p-4 border-t border-border space-y-3">
      ...
      <button disabled={isLoading || !accepted} onClick={() => run("accept_risk")} className="text-sm text-warning disabled:opacity-40">接受风险并继续保存</button>
    </div>
  </div>;
}
