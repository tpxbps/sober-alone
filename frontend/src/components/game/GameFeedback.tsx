...
function SessionFeedback({ sessionId }: { sessionId: string | null }) {
  ...
  const submit = async (recommended: boolean, includeComment = false) => {
    ...
    if (feedback?.recommended === recommended && (!includeComment || feedback.comment === comment)) return;
  ...
  return <section aria-label="剧本体验评价" className="relative max-w-3xl mx-auto border border-border/60 bg-card/50 rounded-xl p-4 space-y-3">
    ...
    <div className="flex flex-wrap gap-2">
      {[true, false].map((recommended) => { const Icon = recommended ? ThumbsUp : ThumbsDown; return <button key={String(recommended)}
        ...
        className={`flex items-center gap-2 px-4 py-2 rounded-lg border text-sm disabled:cursor-default ${feedback?.recommended === recommended ? "border-primary bg-primary/15 text-primary" : "border-border hover:bg-secondary"}`}>
    ...
    {expanded && <div className="space-y-2">
      <textarea aria-label="剧本体验意见" value={comment} onChange={(e) => setComment(e.target.value)} maxLength={1000}
        placeholder={"留下你的体验感受或意见…\n文字仅供维护者查看，不公开展示。"} className="w-full min-h-24 rounded-lg bg-background/60 border border-border p-3 text-sm" />
      <div className="text-right text-xs text-muted-foreground">{comment.length}/1000</div>
